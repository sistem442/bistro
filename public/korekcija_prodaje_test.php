...
if(isset($_POST['submit'])){
	...
	foreach($kolicina_stanje_novo as $id_artikla => $prodata_kolicina) {
		if($diffrence_array_prodaja[$id_artikla] == 0) unset($kolicina_stanje_novo[$id_artikla]);
	}
	...
	print_r ($diffrence_array_prodaja);
			...
			echo 'korekcija  = '.$diffrence_array_prodaja[$id_artikla].'</br>';
			$query4 = "UPDATE stanje 
					  SET kolicina = kolicina + ".$diffrence_array_prodaja[$id_artikla]." 
					  WHERE datum='".$datum."' AND id_artikla=$id_artikla";
  ...
  <form action="<?php echo $_SERVER['PHP_SELF'];?>" name="prodaja" method="post">
    ...
    <?php
            ...
            while ($obj=mysqli_fetch_object($result)){
				...
				//echo $query."</br>";
				$result2 = $mysqli->query($query);
				if ($result2->num_rows == 0) $kolicina = '00000000'; 
				else {
					$temp = $result2->fetch_object();
					$kolicina = $temp->kolicina;
				}
                ...
                <div class="input_wrapper">
                    ...
                    <div style='height:50px; width:200px; float:left;display:inline;'>
                    ...
                    <div style='width:200px;  height:50px; display:inline;color:<?php echo $color;?>'>
